...
You don't create segments by hand. That's why the Segments tab has no create button.
...
The built-in segments are there from the start, even before a brand has any leads. The tab grows as you add tags and custom statuses.

### Create a custom segment by tagging leads
...
- **Name**: the segment name, which is the tag, the status, or the built-in group.
- **Segments**: a category label, such as Tagged Audience, Status, Activity, or Portal Status.
- **Date Created**: when the segment was created.
- **Size**: how many leads are currently in it.

Use **Search Segments**, the filter, and the sort controls to find a segment. **Total Segments** in the top right counts them all.

### What segments unlock
