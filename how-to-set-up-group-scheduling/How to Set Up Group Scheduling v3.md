...
**Subject** is the title that appears on calendar invites. Use `{{name}}` to include the booker's name. **Description** adds optional notes to the invite. **Duration** sets the meeting length in minutes (must be a multiple of 5). Toggle **Add Online Meeting Link** to generate a video call link automatically.
4. Set the **availability schedule** below the form fields. Select your timezone, then for each day of the week, add time windows when the group event is available.
